# PropTrader AI v17.0 — M5 până la H4
...
4. Verifică adresa `/health`. Câmpul `version` trebuie să fie `17.0.0`, iar `analysisTimeframes` trebuie să conțină `5, 15, 30, 60, 240`.
...
4. Creează o singură alertă.
5. La condiție alege indicatorul și **Any alert() function call**.
6. La Webhook URL introdu:

```text
https://ADRESA-TA-RENDER/webhook?key=WEBHOOK_KEY
```

7. Lasă alerta activă. Indicatorul trimite numai lumânări M5 închise; serverul produce intervalele superioare.
