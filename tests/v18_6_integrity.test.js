...
test("v18.6 isolates SMC analytics and validation from legacy data", () => {
...
test("v18.6 blocks misaligned and overlapping SMC plans", () => {
...
test("v18.6 analyzes GER40 and USOIL across aliases, news and Dukascopy history", () => {
  assert.match(server, /GER40: \{ instrument: "deuidxeur"/);
  assert.match(server, /USOIL: \{ instrument: "lightcmdusd"/);
  assert.match(server, /out\.add\("GER40"\)/);
  assert.match(server, /out\.add\("USOIL"\)/);
  assert.match(server, /item\.feedId === "ECB_PRESS"/);
  assert.match(server, /item\.feedId\.startsWith\("EIA_"\)/);
